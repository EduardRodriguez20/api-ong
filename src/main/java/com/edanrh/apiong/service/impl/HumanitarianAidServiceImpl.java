...
public class HumanitarianAidServiceImpl implements HumanitarianAidService {
    ...
    public List<HumanitarianAidDTO> findAll() throws ContentNullException {
        ...
        if (entities.isEmpty()) {
            throw new ContentNullException(ErrorCode.HUMAN_AID_CONTENT_NOT_FOUND, "There isn't humanitarian aid data", HttpStatus.NO_CONTENT);
        }else {
    ...
    public HumanitarianAidDTO save(HumanitarianAidDTO humanitarianAidDTO, Shipment shipment) throws
        ...
        if (head.isEmpty()){
            ...
            throw new NotFoundException(ErrorCode.PROFESSION_CODE_NOT_FOUND, "Profession not found", HttpStatus.NOT_FOUND);
        }else {
            ...
            for (Long doc : humanitarianAidDTO.getDocuments()){
                ...
                if (aid.isEmpty()){
                    throw new NotFoundException(ErrorCode.SANITARY_DOCUMENT_NOT_FOUND, "Doesn't exist a sanitary with document: " + doc, HttpStatus.NOT_FOUND);
                } else if (!aid.get().getIsAvailable()) {
                    throw new BussinesRuleException(ErrorCode.SANITARY_AVAILABLE, "Sanitary: " + doc + " isn't available", HttpStatus.BAD_REQUEST);
                } else if (!aid.get().getProfession().getCodePr().equals(profession.get().getCodePr())) {
                    throw new BussinesRuleException(ErrorCode.SANITARY_PROFESSION, "Sanitary: " + doc + " doesn't participate in the profession", HttpStatus.BAD_REQUEST);
                } else if (!aid.get().getHeadquarter().getCodeHq().equals(head.get().getCodeHq())) {
                        throw new BussinesRuleException(ErrorCode.SANITARY_BELONGS_HEAD, "Sanitary: " + doc + " doesn't belong at the headquarter", HttpStatus.BAD_REQUEST);
                } else {
